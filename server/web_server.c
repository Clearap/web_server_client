...
char documentRoot[] = "/var/www/html/";
...
int main(int argc, char* argv[]){
	...
	if(argc != 2){
		printf("useage : webServer port_number");
		return -1;
	}
...
void do_web(int c_sock){
	...
	p = strtok(NULL, " ");
	if(!strcmp(p, "/")){
		sprintf(url, "%s/index.html", documentRoot);
	}else{
		sprintf(url, "%s%s", documentRoot, p);
	}

	strcpy(c_type, "text/plain");
	for(i = 0; extensions[i].ext != 0; i++){
		len = strlen(extensions[i].ext);
		if(!strncmp(url + strlen(url) - len, extensions[i].ext, len)){
...
void web_log(int type, char s1[], char s2[], int n){
	...
	if((log_fd = open("/var/www/html/web.log", O_CREAT|O_WRONLY|O_APPEND, 0644)) >= 0){
